    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        if (is_multiple) {
            ...
            for (var i = 0; i < files.length; i++) {
                ...
                result[key].push(
                    (process.env.NODE_ENV == 'development' ? process.env.BACK_URL_TEST : process.env.BACK_URL) + '/' + file.destination + file.filename
                )
            }
        } else {
            file.destination = 'files/' + file.destination.split('files/')[1];
            result[key] = (process.env.NODE_ENV == 'development' ? process.env.BACK_URL_TEST : process.env.BACK_URL) + '/' + file.destination + file.filename;
        }
    }
    return result;
}
...
export const differenceTwoDate = (s_d_, e_d_) => {// 'yyyy-mm-dd hh:mm:ss' 로 받음
    let s_d = new Date(s_d_).getTime();
